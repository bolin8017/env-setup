...
function Uninstall-Multiplexer {
    Write-Header 'Uninstall: Multiplexer (zellij)'
    $cfg = (Resolve-Path (Join-Path $PSScriptRoot '../configs/zellij') -ErrorAction Ignore)
    $dest = if ($env:APPDATA) { Join-Path $env:APPDATA 'zellij' } else { Join-Path $HOME '.config/zellij' }
    if ($cfg) {
        Remove-ManagedFile -Dest (Join-Path $dest 'config.kdl') `
            -RepoSrc (Join-Path $cfg.Path 'config.kdl') -Label 'zellij config.kdl'
        Remove-ManagedFile -Dest (Join-Path $dest 'layouts/dev-layout.kdl') `
            -RepoSrc (Join-Path $cfg.Path 'dev-layout.kdl') -Label 'zellij dev layout'
    }
    if (-not (Test-KeepTools)) { Remove-Pkg -Name 'zellij' }
}
