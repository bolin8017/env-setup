...
5. 改完把最後結果寫進呼叫者指定的回報檔（沒指定就寫在目標檔同目錄的 `<檔名>.review.md`），內容三項：改了哪些段落（行號範圍）、hook 或 blocklist 最後一次的結果、你沒改但覺得可疑的句子清單。

## 判準摘要（完整版以 rubric 為準）

- 台灣用語：影片、品質、資訊、軟體、網路、水準、預設、呼叫、函式庫。
- 中文句子用全形標點，不用破折號、不用 emoji。
- 不自創譯名：gate、baseline、prefetch、smoke 這類保留英文並就地解釋一句；A/B 的 arm 寫「組」；不寫「凍結」。
