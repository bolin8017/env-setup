...
**原生 Windows** 安裝對應的同類工具：套件來源用 scoop／winget，prompt 用 Oh My Posh（取代 Powerlevel10k），多工器用 zellij（取代 tmux），Node／Python 用 nvm-windows／uv，並設定 PSReadLine 預測與 Windows Terminal（字型與字型大小 + 預設開 PowerShell 7）。詳見上方 [原生 Windows（PowerShell）](#原生-windowspowershell)。
